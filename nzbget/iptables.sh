...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	...
	# setup route for NZBGet webui using set-mark to route traffic for port 6789 and 6791 to "${docker_interface}"
	echo "6789    webui" >> /etc/iproute2/rt_tables
	echo "6791    webui" >> /etc/iproute2/rt_tables
...
iptables -A INPUT -i "${docker_interface}" -p tcp --dport 6791 -j ACCEPT
iptables -A INPUT -i "${docker_interface}" -p tcp --sport 6789 -j ACCEPT
iptables -A INPUT -i "${docker_interface}" -p tcp --sport 6791 -j ACCEPT
...
if [[ $iptable_mangle_exit_code == 0 ]]; then
	# accept output from NZBGet webui port - used for external access
	iptables -t mangle -A OUTPUT -p tcp --dport 6789 -j MARK --set-mark 1
	iptables -t mangle -A OUTPUT -p tcp --dport 6791 -j MARK --set-mark 1
	iptables -t mangle -A OUTPUT -p tcp --sport 6789 -j MARK --set-mark 1
	iptables -t mangle -A OUTPUT -p tcp --sport 6791 -j MARK --set-mark 1
fi

# accept output from NZBGet webui port - used for lan access
iptables -A OUTPUT -o "${docker_interface}" -p tcp --dport 6789 -j ACCEPT
iptables -A OUTPUT -o "${docker_interface}" -p tcp --dport 6791 -j ACCEPT
iptables -A OUTPUT -o "${docker_interface}" -p tcp --sport 6789 -j ACCEPT
iptables -A OUTPUT -o "${docker_interface}" -p tcp --sport 6791 -j ACCEPT
...
exec /bin/bash /etc/nzbget/start.sh
